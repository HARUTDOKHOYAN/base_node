...
export const IntOutputNodeView: React.FC<Props> = ({ node, selected, onMouseDown, onStartConnect, onDelete }) => {
  ...
  const isCollector = node.instance instanceof IntOutputNode;
  return (
    <div className={`ne-node ${selected ? 'selected' : ''}`} style={{ left: node.position.x, top: node.position.y }} onMouseDown={(e) => onMouseDown(e, node.id)}>
      ...
      {isCollector && (
        <div className="ne-output">
          <label className="ne-output-label">Value</label>
          <input className="ne-output-input" type="text" readOnly value={(inputs[0]?.Value ?? '') as any} onMouseDown={(e) => e.stopPropagation()} />
        </div>
      )}
    </div>
  );
