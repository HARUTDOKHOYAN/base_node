...
interface Props {
  ...
  onChangeOutputValue: (nodeId: string, index: number, value: number) => void;
}

export const IntInputNodeView: React.FC<Props> = ({ node, selected, onMouseDown, onStartConnect, onDelete, onChangeOutputValue }) => {
  ...
  return (
    <div
      className={`ne-node ${selected ? 'selected' : ''}`}
        ...
        <div className="ne-node-ports-col right">
          {outputs.map((p, i) => (
            <div className="ne-port" key={`out-${i}`}>
              <span className="ne-port-name">{p.Name}</span>
              <input className="ne-port-input" type="number" value={p.Value ?? ''} onMouseDown={(e) => e.stopPropagation()} onChange={(e) => { const v = e.currentTarget.value === '' ? NaN : Number(e.currentTarget.value); if (!Number.isNaN(v)) onChangeOutputValue(node.id, i, v); }} />
